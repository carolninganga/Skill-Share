$(document).ready(function () {
  ...
  $.get("/api/profile", function (data) {
    for (var i = 0; i < data.length; i++) {
      ...
      wellSection.append(`<div class='portfolio-card'> <div class='portfolioContent'><img class='rounded-circle avatarImg' src='https://api.adorable.io/avatars/100/${avatar}'><h2 class='portfolioTitle'>${data[i].First_Name} ${data[i].Last_Name}</h2><p class='cardCategory'> ${data[i].City} ${data[i].Zip_Code} </p><p>${data[i].Bio} <br> </p> <h5 class='tags'> ${data[i].Skill_1}</h5> <h5 class='tags'>${data[i].Skill_2}</h5><a class='readMore' href='/detail'>Read More</a></div></div>`);
